...
public class SQLiteVanProductHandler extends SQLiteDatabaseHandler{
    Context context;
    public SQLiteVanProductHandler(Context context) {
        super(context);
    ...
    public void deleteAll(){
        ...
        db.execSQL("delete from "+ TABLE_VAN_PRODUCT);
        db.close();
    }

    public void addProductList(List<VanProductModel> productsModels) {
        for(int i=0;i<productsModels.size();i++) {
            ...
            long chack = db.insertOrThrow(TABLE_VAN_PRODUCT, null, values);
            if(chack == -1){i--;}
            db.close();
    ...
    public List<ProductsModel> getAllProduct() {
        ...
        // Select All Query
        String selectQuery = "SELECT  * FROM " + TABLE_VAN_PRODUCT;
